        ...
        a.extend(a.fn, {
            uploadify: function(b) {
                a(this).each(function() {
					...
					f.successTimeout = settings.successTimeout;
                    ...
                    a(this).bind("uploadifySelect", {
                        ...
                        if (j.data.action(j, h, i) !== false) {
                            var k = Math.round(i.size / 1024 * 100) * .01;
                            var l = "KB";
                            if (k > 1e3) {
                                k = Math.round(k * .001 * 100) * .01;
                                l = "MB";
                            }
                            ...
                            a("#dialog-uploadify").show();
                            a(queue).html(fileName + " (" + k + l + ')<span id="percentage"></span>');
                            a(".turbo-upload").html('<a class="ng-binding" href="javascript:jQuery(\'#' + a(this).attr("id") + "').uploadifyCancel('" + h + "')\">取消</a>");
                        }
                    });
                    ...
                    a(this).bind("uploadifyCancel", {
                        ...
                        if (l.data.action(l, h, k, m, j) !== false) {
                            var i = j == true ? 0 : 250;
                            a("#speed-uploadify").fadeOut(i, function() {
                                a(this).show().text("已取消上传");
                                a(".growing").css("width", "0%");
                                a(".turbo-upload").hide();
                            });
                        }
                    });
                    ...
                    a(this).bind("uploadifyError", {
                        ...
                        if (l.data.action(l, h, k, j) !== false) {
                            ...
                            c.push(i);
                            a("#percentage").text(" - " + j.type + " Error");
                        }
                    });
                    a(this).bind("uploadifyProgress", {
                        ...
                        if (j.data.action(j, h, i, k) !== false) {
                            a(".growing").css("width", k.percentage + "%");
...
$(document).ready(function() {
    $("#uploadify").uploadify({
        uploader: in_path + "static/pack/upload/uploadify.swf",
        script: in_path + "source/pack/upload/uplog.php",
        wmode: "transparent",
        folder: "UploadFile",
        method: "POST",
        scriptData: {
        ...
        buttonImg: uploadify_img,
        width: "110",
        height: "30",
        queueID: "speed-uploadify",
        auto: true,
        multi: false,
		successTimeout:600,
        fileExt: "*.apk,.ipa",
        fileDesc: "选择*.ipa文件",
        sizeLimit: in_size * 1024 * 1024,
        onError: function(a, b, c, d) {
            if (d.status == 404) {
                $("#speed-uploadify").text("上传异常，请重试！");
            } else if (d.type === "HTTP") {
                $("#speed-uploadify").text("error " + d.type + " : " + d.status);
            } else if (d.type === "File Size") {
                $("#speed-uploadify").text("上传失败，大小不能超过" + in_size + "MB！");
            } else {
                $("#speed-uploadify").text("error " + d.type + " : " + d.text);
            }
            $(".growing").css("width", "0%");
            $(".turbo-upload").hide();
        },

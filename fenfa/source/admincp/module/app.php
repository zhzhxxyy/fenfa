...
switch($action){
	...
	case 'keyword':
		...
		main("select * from ".tname('app')." where in_name like '%".$key."%' or in_uname like '%".$key."%' order by in_addtime desc",20);
		break;
	default:
		main("select * from ".tname('app')." order by in_addtime desc",20);
		break;
	}
?>
</body>
...
function EditBoard($Arr,$url,$arrname){
	...
	$in_type = !is_numeric($Arr[2]) ? 0 : $Arr[2];
	$in_mnvs = $Arr[3];
	$in_form = $Arr[4];
	$in_size = $Arr[5];
	$in_bid = $Arr[6];
...
<div class="container">
<script type="text/javascript">parent.document.title = 'Ear Music Board 管理中心 - 应用 - <?php echo $arrname; ?>应用';if(parent.$('admincpnav')) parent.$('admincpnav').innerHTML='应用&nbsp;&raquo;&nbsp;<?php echo $arrname; ?>应用';</script>
...
<tr><td class="longtxt">应用标识：<input type="text" class="txt" value="<?php echo $in_bid; ?>" name="in_bid" id="in_bid"></td></tr>
...
<tr><td class="longtxt">图标文件：<input type="text" class="txt" value="<?php echo $in_icon; ?>" name="in_icon" id="in_icon"></td></tr>
<tr><td class="longtxt diffcolor3">配置地址：<input type="text" class="txt" value="<?php echo $in_plist; ?>" name="in_plist" id="in_plist"></td><td><div class="rssbutton"><input type="button" value="上传应用" onclick="pop.up('no', '上传应用', 'source/pack/upload/open.php', '406px', '180px', '175px');" /></div></td></tr>
<tr><td><input type="submit" class="btn" name="form2" id="btnsave" value="提交" onclick="return CheckForm();" /><?php if($_GET['action']=="edit"){ ?><input type="hidden" name="in_addtime" value="<?php echo $in_addtime; ?>"><input class="checkbox" type="checkbox" name="in_edittime" id="in_edittime" value="1" checked /><label for="in_edittime">更新时间</label><?php } ?></td></tr>
</table>
</form>
</div>
...
<div class="container">
<?php if(empty($action)){echo "<script type=\"text/javascript\">parent.document.title = 'Ear Music Board 管理中心 - 应用 - 所有应用';if(parent.$('admincpnav')) parent.$('admincpnav').innerHTML='应用&nbsp;&raquo;&nbsp;所有应用';</script>";} ?>
<?php if($action=="keyword"){echo "<script type=\"text/javascript\">parent.document.title = 'Ear Music Board 管理中心 - 应用 - 搜索应用';if(parent.$('admincpnav')) parent.$('admincpnav').innerHTML='应用&nbsp;&raquo;&nbsp;搜索应用';</script>";} ?>
...
<td><a href="<?php echo geticon($row['in_icon']); ?>" id="thumb<?php echo $row['in_id']; ?>"><img src="<?php echo geticon($row['in_icon']); ?>" width="25" height="25" /></a></td>
...
<?php
	...
	function Edit(){
		...
		if($row=$db->getrow($sql)){
			$Arr=array($row['in_name'],$row['in_uname'],$row['in_type'],$row['in_mnvs'],$row['in_form'],$row['in_size'],$row['in_bid'],$row['in_bsvs'],$row['in_bvs'],$row['in_nick'],$row['in_team'],$row['in_icon'],$row['in_plist'],$row['in_addtime']);
		}
	...
	function SaveAdd(){
		...
		if($row=$db->fetch_array($result)){
			$db->query("Insert ".tname('app')." (in_name,in_uid,in_uname,in_type,in_size,in_form,in_mnvs,in_bid,in_bsvs,in_bvs,in_nick,in_team,in_icon,in_plist,in_hits,in_addtime) values ('".$in_name."',".$row['in_userid'].",'".$row['in_username']."',".$in_type.",'".$in_size."','".$in_form."','".$in_mnvs."','".$in_bid."','".$in_bsvs."','".$in_bvs."','".$in_nick."','".$in_team."','".$in_icon."','".$in_plist."',0,'".date('Y-m-d H:i:s')."')");
	...
	function SaveEdit(){
		...
		$result=$db->query("select * from ".tname('user')." where in_username='".$in_uname."'");
		if($row=$db->fetch_array($result)){
			$db->query("update ".tname('app')." set in_name='".$in_name."',in_uid=".$row['in_userid'].",in_uname='".$row['in_username']."',in_type=".$in_type.",in_size='".$in_size."',in_form='".$in_form."',in_mnvs='".$in_mnvs."',in_bid='".$in_bid."',in_bsvs='".$in_bsvs."',in_bvs='".$in_bvs."',in_nick='".$in_nick."',in_team='".$in_team."',in_icon='".$in_icon."',in_plist='".$in_plist."',in_addtime='".$datetime."' where in_id=".$in_id);

...
<body>
<div class="container">
<script type="text/javascript">parent.document.title = 'Ear Music Board 管理中心 - 工具 - 站点备份';if(parent.$('admincpnav')) parent.$('admincpnav').innerHTML='工具&nbsp;&raquo;&nbsp;站点备份';</script>
...
<?php
	...
	$result = $db->list_tables(IN_DBNAME);
	if($result){
	...
	$tableresult = $db->list_tables(IN_DBNAME);
	if($tableresult){
		$bakstr = '';
		$batch = 0;
		while($tablerow = mysql_fetch_row($tableresult)){
			...
			if($prefix[0] == str_replace('_', '', IN_DBTABLE)){
				...
				$fieldresult = $db->list_fields(IN_DBNAME, $tablerow[0]);
				$i = 0;
				if($fieldresult){
					...
					for($j = 0; $j < count($result); $j++){
						...
						for($k = 0; $k <= $fsd; $k++){
							if($k < $fsd){
								$line .= "'".mysql_real_escape_string($result[$j][$fs[$k]])."',";
							}else{
								$line .= "'".mysql_real_escape_string($result[$j][$fs[$k]])."');\r\n";
							}
						}
